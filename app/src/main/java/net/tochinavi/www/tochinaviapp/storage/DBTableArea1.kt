...
import net.tochinavi.www.tochinaviapp.entities.DataArea1


class DBTableArea1(context: Context) {

    private val TAG = "DBTableArea1"
    private val tableName = DataArea1.TABLE_NAME
    ...
    fun getAll(dBHelper: DBHelper): ArrayList<DataArea1> {
        var c: Cursor? = null
        var datas: ArrayList<DataArea1> = ArrayList()
    ...
    fun getData(dBHelper: DBHelper, id: Int): DataArea1 {
        var c: Cursor? = null
        var data = DataArea1()
    ...
    // カラム全取得のみで使える
    fun getResult(c: Cursor): DataArea1 {
        return DataArea1(
    ...
    /** insert **/
    fun insert(dBHelper: DBHelper, data: DataArea1): Int {
        val cv = ContentValues()
        cv.put(DataArea1.COL[0], data.id)
        cv.put(DataArea1.COL[1], data.name)
        val rowId: Long = dBHelper.db!!.insert(tableName, null, cv)
        return  rowId.toInt()
    }

    /** update **/
    fun update(dBHelper: DBHelper, data: DataArea1, condition: String): Boolean {
        val cv = ContentValues()
        cv.put(DataArea1.COL[1], data.name)
